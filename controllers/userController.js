...
const register= async(req,res)=>{
    try{
        ...
        if(newUser) res.restatus(400).json({msg:"user already exist , try to connect"})
        else {
        ...
        res.states(500).json({msg:"somthing went wrong",error:error.message})
...
const login= async(req,res)=>{
    try{
        ...
        if (!existUser) res.status(400).json({msg:"user does not exist,try to register"})
          ...
          res.ststus(201).json({msg:"login sucess",token:token, person:existUser})        
        }
